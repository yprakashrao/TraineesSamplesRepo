  ...
  promise
    ...
    .then((data) => {
      ...
      tdata = `<td>${data.id}</td>
        ...
        <td>${data.symptoms}</td>
        <td>${data.add_info}</td>
        <td><button id="edit-btn" onclick="editData(${data.id})">edit</button></td>
        <td><button id="delete-btn" onclick="deleteData(${data.id})">delete</button></td>`;
...
function update(bodydata, id) {
  ...
  let promise = fetch("http://192.168.0.225:8080/update", {
    method: "PUT",
    headers: {
